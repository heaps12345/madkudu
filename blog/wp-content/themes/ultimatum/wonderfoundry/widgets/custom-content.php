...
function widget($args, $instance) {
	...
	*/
	...
	if (isset($loopfile)){
		if(file_exists(ULTLOOPBUILDER_DIR.DS.$loopfile) ) { //Wonder Loop include
		...
		?>
						     	<article class="post post-<?php echo $post->ID;?> ultimatepost-custom <?php echo $col_class; if($gps) echo " last";?>" >
			        		   ...
			        		   <?php } ?>
				        		  <?php if($video && $instance["mvideo"]=='true') { ?>
...
function form($instance) {
      ...
      <option value='ASC'             <?php selected($orderdir, 'ASC'             );?>><?php _e( 'Ascending'             , 'ultimatum') ?></option>
		...
		<p>
			<label for="<?php echo $this->get_field_id('mimgpos'); ?>"><?php _e('Image Position', 'ultimatum') ?>:</label> <i>For Full image and columns 2 or 2+</i>
		...
		<p>
			<label for="<?php echo $this->get_field_id('mvideo'); ?>"><?php _e('Replace Featured Image with gallery or Video', 'ultimatum') ?>:</label> <i>Works for non Gallery views only</i>
			<select name="<?php echo $this->get_field_name('mvideo'); ?>" id="<?php echo $this->get_field_id('mvideo'); ?>">
			<option value="false" <?php selected($mvideo,'true');?>>ON</option>
		...
		<p>
		<label for="<?php echo $this->get_field_id('excerptlength'); ?>"><?php _e('Excerpt Length(chars)', 'ultimatum') ?>:</label>
		...
		<option value="aimage" <?php selected($mmeta,'aimage');?>><?php _e('After Image', 'ultimatum') ?></option>
