	...
	if(is_singular()){
		...
		if($imgsrc){ ?>
	        ...
	        if($instance["gallery"]=='true'){
				...
				<img src="<?php echo $imgsrc;?>" alt="<?php the_title();?>" /><?php 
			}
...
function ultimatum_do_post_title($args,$instance) {
	...
	if ( is_singular() && $instance["title"]=='true' ){
		$tag = get_ultimatum_option('tags', 'multi_article');
		$title = sprintf( '<'.$stag.' class="post-header">%s</'.$stag.'>', $title );
		echo "$title \n";
	}
	if ( !is_singular() && $instance["mtitle"]=='true' ) {
		$title = sprintf( '<'.$tag.' class="post-header"><a href="%s" title="%s" rel="bookmark" class="post-title">%s</a></'.$tag.'>', get_permalink(), the_title_attribute( 'echo=0' ), apply_filters( 'ultimatum_post_title_text', $title ) );
		echo "$title \n";
	} elseif(!is_singular() && $instance["mtitle"]=='nolink') {
		$title = sprintf( '<'.$tag.' class="post-header">%s</'.$tag.'>', $title );
		echo "$title \n";
	}
		...
		if ( $instance['excerpt']=='true' ) { 
			...
			if($instance['mreadmore']!='false'){
				if($instance['mreadmore']!='after'){
					...
					echo '<br /><span style="float:'.$instance['mreadmore'].'"><a href="'.get_permalink().'">'.$loop_text_vars[$instance['rmtext']].'</a></span><br />';
				} else {
					echo ' <a href="'.get_permalink().'">'.$loop_text_vars[$instance['rmtext']].'</a>';
				}
			}
			echo '</p>';
...
function ultimatum_prev_next_posts_nav() {
	...
	$prev = $prev_link ? '<ul class="alignleft"><li>' . $prev_link . '</li></ul>' : '';
	$next = $next_link ? '<ul class="alignright"><li>' . $next_link . '</li></ul>' : '';
	$nav = '<div class="pagination">' . $prev . $next . '</div><!-- end .navigation -->';
