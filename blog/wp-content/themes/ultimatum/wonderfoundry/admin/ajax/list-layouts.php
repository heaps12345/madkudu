...
<h3><?php _e('Full Layouts','ultimatum');?></h3>
	...
	foreach($full as $layout){
		...
		echo '<a href="./admin.php?page=wonder-layout&task=edit&theme='.$theme.'&layoutid='.$layout["id"].'" class=""><i class="icon-edit"></i> '.__('Edit ','ultimatum').'</a>  | ';
		echo '<a class="clonelayout" data-layout="'.$layout["id"].'"><i class="icon-copy"></i> '.__('Clone','ultimatum').'</a>  |  ';
		echo '<input type="hidden" name="action" value="" class="action" />';
		echo '<input type="hidden" name="layoutid" value="'.$layout["id"].'" />';
		echo '<a class=" deletelayout" data-layout="'.$layout["id"].'"><i class="icon-trash"></i> Delete</a> | ';
		if($layout['default']!='1'){
			echo'<a class="setdefault" data-template="'.$theme.'" data-layout="'.$layout["id"].'"><i class="icon-ok-sign"></i> '.__('Set Default','ultimatum').'</a>';
		}
	...
	foreach ($resultp as $layout){
		...
		echo '<div class="row-actions">';
		echo '<form method="post" action="" class="layoutactions">';
		echo '<a href="./admin.php?page=wonder-layout&task=edit&theme='.$theme.'&layoutid='.$layout["id"].'" ><i class="icon-edit"></i> '.__('Edit ','ultimatum').'</a> | ';
		echo '<a class="clonelayout" data-layout="'.$layout["id"].'"><i class="icon-copy"></i> '.__('Clone','ultimatum').'</a> | ';
		echo '<input type="hidden" name="action" value="" class="action" />';
		echo '<input type="hidden" name="layoutid" value="'.$layout["id"].'" />';
		echo '<a class="deletelayout" data-layout="'.$layout["id"].'"><i class="icon-trash"></i> Delete</a>';
