	...
	switch ($task){ //
		...
		case 'edit':
			...
			if($_POST){
				global $wpdb;
				if(isset($_POST['id'])){
					$query = "REPLACE INTO `".ULTIMATUM_TABLE_TEMPLATES."` VALUES ('".$_POST['id']."','".$_POST['name']."','".$_POST['width']."','".$_POST['margin']."','".$_POST['mwidth']."','".$_POST['mmargin']."','".$_POST['swidth']."','".$_POST['smargin']."','".$_POST['gridwork']."','".$_POST['swatch']."','".$_POST['type']."','".$_POST['dcss']."','".$_POST['default']."','".$_POST['theme']."')";
				} else {
    ...
    <thead>
		...
		foreach($templates as $template){
			...
			echo '<tr'.$class.'>';	
				...
				echo '<a href="./admin.php?page=wonder-templates&task=edit&template_id='.$template->id.'" class=""><i class="icon-edit"></i> '.__('Edit','ultimatum').'</a>  |  ';
						if($template->default!=1){
							echo '<a href="#" class="defaulttemplate" data-id="'.$template->id.'"><i class="icon-cogs"></i> '.__('Set Default','ultimatum').'</a>  |  ';
						}
						if(function_exists('Ultimatum_Exporter')){
							echo '<a href="./index.php?page=ultimatum-export&task=template&template_id='.$template->id.'&TB_iframe=1&width=770&height=480" class="thickbox"><i class="icon-cloud-download"></i> '.__('Export','ultimatum').'</a>  |  ';
						}
						echo '<a href="#" class="deletetemplate" data-id="'.$template->id.'" ><i class="icon-trash"></i> Delete</a>  |  ';
						
						echo '<a href="./admin.php?page=wonder-templates&task=row-styles&template_id='.$template->id.'" class=""><i class="icon-th-large"></i> '.__('Extra Row Styles','ultimatum').'</a>  |  ';
					...
					echo '</div>';
				echo'</th>';echo '<td>';
				echo '<a href="./admin.php?page=wonder-css&template_id='.$template->id.'" class=""><i class="icon-cogs"></i> '.__('CSS','ultimatum').'</a>';
				echo'</td>';echo '<td>';
				echo '<a href="./admin.php?page=wonder-layout&theme='.$template->id.'" class=""><i class="icon-desktop"></i> '.__('Layouts','ultimatum').'</a>';
				echo'</td>';
			echo '</tr>';
			$count++;
		}
...
function RowStyles(){
	...
	<table class="table table-bordered">
		...
		<?php 
			...
			<td><a class="btn btn-small" href="./admin.php?page=wonder-templates&task=row-edit&template_id=<?php echo $_GET['template_id'];?>&slug=<?php echo $extrarow->slug;?>"><i class="icon-edit"></i>Edit</a>
			</tr>
			<?php 
			}
		?>
	...
	$theme_root =  get_theme_root();
 	...
 	if(is_writable($theme_root)):
	 	...
	 	<table class="table table-bordered">
			...
			<tr>
				<th colspan="3">
					<button class="btn btn-info"><i class="icon-save"></i> <?php _e('Save','ultimatum');?></button>
				</th>
			</tr>
			</tfoot>
			<tbody>
			...
			<tr>
				<td><?php _e('Child Theme Slug','ultimatum'); ?>: </td>
				<td width="200"><input type="text" name="slug" value="" /></td>
				<td><?php _e('Short name of your child theme only alpha numeric and noncapital letters please this needs to be unique for your theme otherwise ther might be conflicts. Eg. my_cool_theme','ultimatum');?></td>
			</tr>
			<tr>
				...
				<td><?php _e('If non selected an empthe child theme with no layouts will be created','ultimatum');?></td>
			</tr>
 	...
 	// create the folder
 	$themedir = $theme_root.DS.$_POST['slug'];
	mkdir($themedir); 	
