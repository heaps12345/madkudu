...
function ultimatum_menu_access_rights(){
	...
	if(is_multisite()){
		global $blog_id;
		if(is_array($option['disable_sites']) && count($option['disable_sites'])!=0 && in_array($blog_id, $option['disable_sites']) && !is_super_admin()){
			return false;
		} else {
			return true;
		}
		...
		$user_ID = get_current_user_id();
		if($option['disable_menus']=='true' && $option['allowed_user']!=$user_ID ){
			return false;
		} else {
			return true;
		}
